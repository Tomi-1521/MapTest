...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!move)
        {
            ...
            if (Input.GetKey("up") && existsSquare(xIndex, zIndex, "up"))
            {
                ...
                target = current + new Vector3(0.0f, 0.0f, MC.spaceScale.z);
                move = true;
                zIndex--;
            }
            ...
            else if (Input.GetKey("right") && existsSquare(xIndex, zIndex, "right"))
            {
                MapCreator MC = refObj.GetComponent<MapCreator>();
                current = this.transform.position;
                target = current + new Vector3(MC.spaceScale.x, 0.0f, 0.0f);
                move = true;
                xIndex++;
            }
    ...
    void LoadMap()
    {
        ...
        foreach (var c in mapTextData)
        {
            ...
            else if (char.IsDigit(c) && readLine)
            {
                col++;
            }
            n++;
    ...
    bool existsSquare(int xIndex, int zIndex, string direction)
    {
        ...
        {
            case "up":
                if (zIndex > 0 && mapData[zIndex - 1, xIndex] == '1')
                    return true;
                else
                    return false;
            case "down":
                if (zIndex < mapRow - 1 && mapData[zIndex + 1, xIndex] == '1')
                    return true;
                else
                    return false;
            case "right":
                if (xIndex < mapCol - 1 && mapData[zIndex, xIndex + 1] == '1')
                    return true;
                else
                    return false;
            case "left":
                if (xIndex > 0 && mapData[zIndex, xIndex - 1] == '1')
                    return true;
                else
                    return false;
            default:
